...
- 호스트 포트 `8080`이 컨테이너 `8080`으로 연결된다
...
**빈 상태로 시작**: `http://<NAS IP>:8080` 접속 → 사번 `admin`으로 신규 등록 → Quality Admin 자동 승격
...
공유기 관리 화면에서 외부 포트(예: `18080`) → NAS 내부 IP:`8080`으로 포워딩한다.
