...
> 일반 사용자는 비밀번호 없이 사번만으로 로그인하며, HTTPS·요청 제한 같은 인터넷 노출 방어가 없다.
> 아래 절차로 외부에 열면 **사번을 아는(또는 추측한) 누구나 그 사람으로 로그인할 수 있다.**
> (Quality Admin 승격은 별도 관리자 비밀번호가 있어야 가능하도록 분리되어 있다 — 아래 "관리자 비밀번호 설정" 참고.)
> 실제 결함 데이터나 개인정보를 올리지 말고, 데모/시연 목적으로만 짧게 열어두는 것을 권장한다.
> 최소한의 안전장치가 필요하면 부록 A(Reverse Proxy 기본 인증)를 참고한다.

### 관리자 비밀번호 설정 (필수)

`docker-compose.yml`의 `DMS_ADMIN_PASSWORD` 값을 배포 전에 반드시 바꾼다. 이 값이 비어 있으면
Quality Admin 로그인(`#/admin-login`) 자체가 거부된다.

```yaml
environment:
  - DMS_ADMIN_PASSWORD=여기에_직접_정한_비밀번호
```

일반 사용자는 여전히 사번만으로 로그인하지만, Quality Admin 권한은 이 비밀번호를 아는 사람만
`#/admin-login` 화면(일반 로그인 화면 하단의 작은 "· Admin" 링크)에서 사번 + 이 비밀번호로 로그인해야
얻을 수 있다.
